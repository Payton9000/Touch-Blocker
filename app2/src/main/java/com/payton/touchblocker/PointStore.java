...
public class PointStore {
    ...
    public static int getGlobalSizePx(Context context) {
        ...
        if (preferences.contains(KEY_GLOBAL_SIZE)) {
            return preferences.getInt(KEY_GLOBAL_SIZE, 0);
        }
        float density = context.getResources().getDisplayMetrics().density;
        return Math.max(1, Math.round(ScreenProfile.DEFAULT_GLOBAL_DIAMETER_DP * density));
    }
